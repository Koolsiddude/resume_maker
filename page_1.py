...
for i in range(num_jobs):
    with st.expander(f"Job {i + 1}"):
        position = st.text_input(f"Position {i + 1}", key="Position {i + 1}")
        company = st.text_input(f"Company {i + 1}", key="Company {i + 1}")
        start_date = st.text_input(f"Start Date {i + 1}", key="Start Date {i + 1}")
        end_date = st.text_input(f"End Date {i + 1} (or 'Present')", key = "End Date {i + 1}")
...
for i in range(num_education):
    with st.expander(f"Education {i + 1}"):
        degree = st.text_input(f"Degree {i + 1}", key="Degree {i + 1}")
        institution = st.text_input(f"Institution {i + 1}", key="Institution {i + 1}")
        edu_start_date = st.text_input(f"Start Date {i + 1}", key="Start Date {i + 1} another")
        edu_end_date = st.text_input(f"End Date {i + 1}", key="End Date {i + 1} another")
        details = st.text_area(f"Details {i + 1}",key="Details {i + 1}")
        education.append({
            "degree": degree,
...
for i in range(num_projects):
    with st.expander(f"Project {i + 1}"):
        project_name = st.text_input(f"Project Name {i + 1}",key="Project Name {i + 1}")
        project_date = st.text_input(f"Project Date {i + 1}",key="Project Date {i + 1}")
        project_description = st.text_area(f"Project Description {i + 1}",key="Project Description {i + 1}")
        projects.append({
            "name": project_name,
            "date": project_date,
