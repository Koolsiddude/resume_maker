...
for i in range(num_jobs):
    with st.expander(f"Job {i + 1}"):
        ...
        end_date = st.text_input(f"End Date {i + 1} (or 'Present')")
...
for i in range(num_education):
    with st.expander(f"Education {i + 1}"):
        ...
        edu_start_date = st.text_input(f"Start Date {i + 1}")
        edu_end_date = st.text_input(f"End Date {i + 1}")
...
for i in range(num_projects):
    with st.expander(f"Project {i + 1}"):
        ...
        project_date = st.text_input(f"Project Date {i + 1}")
...
certifications = st.text_area("Certifications (Separate by commas)").split(",")
