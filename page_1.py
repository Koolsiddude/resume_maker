...
<html lang="en">
  ...
  <body>
    <div class="container">
      <img src="vbs-logo-removebg-preview.png" alt="Institution Logo" style="float: right; width: 30vh; height: auto;"></img>
      <div class="header">
        <h1>{{name}}</h1>
        <p>
          Email:
   ...
   if "include_work_experience" in params:
       template += """ 
      ...
      """
   if "include_intern_experience" in params:
       template += """
             ...
             <tr class="live-project">
              ...
              <td id="des" colspan="4">
                <table class="live-project">
                  ...
                  <tr>
                    <td> &bull; {{ project.name }} {{ project.company_name }} {{ project.date }}</td>
                  </tr>
                  <tr>
        ...
        <tr class="live-project">
              ...
              <td id="des" colspan="4">
                <table class="live-project">
                  ...
                  <tr>
                    <td>&bull; {{ pos_rep.name }}</td>
                  </tr>
                  <tr>
                    <td> &nbsp; {{ pos_rep.description }}</td>
                  </tr>
                  {% endfor %}
                </table>
              </td>
...
if include_pos:
    ...
    for i in range(num_pos):
        with st.expander(f"Responsibility {i + 1}"):
            pos_name = st.text_input(f"Title",key=f"pos_name {i + 1}")
            pos_description = st.text_area(f"Description (Separate by semi-colon ' ; ')",key=f"pos_description {i + 1}")
            pos.append({
                "name": pos_name,
                "num": num_pos,
...
if include_soc_proj:
    ...
    for i in range(num_soc_proj):
        with st.expander(f"Social Project {i + 1}"):
            soc_proj_name = st.text_input(f"Title",key=f"res_title {i + 1}")
